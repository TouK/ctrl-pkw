...
public interface WardRepository extends PagingAndSortingRepository<Ward, Long> {
    ...
    @Query("select w from Ward w join w.votings v where w.location is not null and v = :voting  and distance_sphere(w.location, :point) is not null order by distance_sphere(w.location, :point)")
    Page<Ward> findOrderedByDistance(@Param("voting") Voting voting, @Param("point") Point point, Pageable pageable);

    @Query("select w from Ward w join w.votings v where w.location is not null and v = :voting and distance_sphere(w.location, :point) <= :radius order by distance_sphere(w.location, :point)")
    ...
    @Query("select w from Ward w join w.votings v where w.location is not null and v = :voting and distance_sphere(w.location, :point) <= distance_sphere(:distantPoint, :point) order by distance_sphere(w.location, :point)")
